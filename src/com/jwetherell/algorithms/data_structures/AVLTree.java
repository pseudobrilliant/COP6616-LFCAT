...
public class AVLTree<T extends Comparable<T>> extends BinarySearchTree<T> {
    ...
    public AVLTree(AVLTree<T> orig)
    {
        super(orig);
        if(orig.root != null)
        {
            this.root = (AVLNode<T>)orig.root.copy(null);
            this.size = this.root.size();
        }
    }

    public AVLTree(AVLNode<T> orig) {
        super(getCreator());
        if(orig != null)
        {
            this.root = orig.copy(null);
            this.size = orig.size();
        }
    }

    private static <T extends Comparable<T>> INodeCreator<T> getCreator()
    {
       return new BinarySearchTree.INodeCreator<T>() {
            /**
             * {@inheritDoc}
             */
            @Override
...
*/
    ...
    protected static class AVLNode<T extends Comparable<T>> extends Node<T> {
        ...
        AVLNode<T> copy(Node _parent)
        {
            AVLNode<T> newMe = new AVLNode<T>(_parent, this.id);

            Node left = null;
            Node right = null;

            if (this.lesser != null) {
                left = this.lesser.copy(newMe);
            }
            if (this.greater != null) {
                right = this.greater.copy(newMe);
            }
    ...
    public static class Pair<T extends Comparable<T>>  {
        ...
        public AVLTree<T> getLesser() {
            return left;
        }

        public AVLTree<T> getGreater() {
            return right;
        }
